...
export const findNearbyHealthyOptions = async (lat: number, lng: number) => {
  const prompt = `Find nearby healthy grocery stores (like Tesco, Sainsbury's, Waitrose) and healthy restaurants in this area (${lat}, ${lng}). 
  For groceries, list some specific healthy items with estimated prices. 
  For restaurants, suggest specific healthy dishes.`;
